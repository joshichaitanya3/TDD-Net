...
# change output channel for last fully connected layer
# according to number of classes
num_ftrs = model.fc.in_features
model.fc = nn.Conv2d(in_channels = 512, 
                     out_channels = len(classes)+1, 
                     kernel_size = 1, 
                     padding = 0)
# number of classes plus negative samples 
# model.fc = nn.Linear(num_ftrs, len(classes)+1) 
...
model = train_model(model, criterion, optimizer, exp_lr_scheduler, 
                    data_transform, train_num = train_num, 
                    test_num = test_num, non_pos_ratio = non_pos_ratio, 
                    window_size = window_size, batch_size = batch_size, 
                    device = device, classes = classes, 
                    df_train_path = df_train_path, 
                    df_test_path = df_test_path, num_epochs=num_epochs, 
                    method = method, checkpoint_path = checkpoint_path)
