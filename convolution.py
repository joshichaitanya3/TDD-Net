...
df_yolo = pd.read_csv('/home/rliu/github/TDD-Net/yolo2_dm/'
                      'results/train_yolo.csv', sep=' ')
df_train = pd.read_csv('/home/rliu/yolo2/v2_pytorch_yolo2/data/'
                       'an_data/VOCdevkit/VOC2007/csv_labels/train.csv',
                       sep=" ")
...
model_hard.eval()
model_FNN = torch.load('/home/rliu/TDD-Net/models/python/ml/'
                       '2x100_28epo_yolo_01-13-18.model')
model_FNN.eval()
...
for i in df_train.image_index.unique():
    testset = defectDataset_convolution(
        image_index = i, 
        img_path='/home/rliu/TDD-Net/data/',
        stride = 2, window_size=45, 
    ...
    with torch.no_grad():
        ...
        confidence_neg = np.array([]) 
        confidence_pos_o = np.array([]) 
        confidence_nuc = np.array([]) 
        confidence_non = np.array([])
        iii = 0
        for inputs in testloader:
            ...
            confidence_pos = np.append(confidence_pos, 
                                       confidences[:,0].cpu().numpy())
            confidence_neg = np.append(confidence_neg, 
                                       confidences[:,1].cpu().numpy())
            confidence_pos_o = np.append(confidence_pos_o, 
                                         confidences[:,2].cpu().numpy())
            confidence_nuc = np.append(confidence_nuc, 
                                       confidences[:,3].cpu().numpy())
            confidence_non = np.append(confidence_non, 
                                       confidences[:,4].cpu().numpy())
...
print(f'Detecting complete in {(time_elapsed // 60):.0f}m '
